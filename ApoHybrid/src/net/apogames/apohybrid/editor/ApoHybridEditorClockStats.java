...
public class ApoHybridEditorClockStats {
	...
	private final ApoHybridEntityClock clock;
	
	public ApoHybridEditorClockStats(final ApoHybridEntityClock clock) {
		this.clock = clock;
	...
	public final ApoHybridEntityClock getClock() {
		return this.clock;
	}
	...
	private void setCurrentPosition() {
		...
		if (this.x < 0) {
			this.x = 0;
		} else if (this.x + this.width >= ApoHybridConstants.GAME_WIDTH) {
			this.x = ApoHybridConstants.GAME_WIDTH - this.width;
		}
	...
	public void render(BitsGLGraphics g) {
		...
		String s = "-";
		for (int i = 0; i < 4; i++) {
			g.drawText(s, this.x + 65, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + i * 50);
		}

		g.drawText(s, this.x + this.width/2 + 20, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + 200);
		g.drawText(s, this.x + 20, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + 200);
		s = "+";
		for (int i = 0; i < 4; i++) {
			g.drawText(s, this.x + width - 75, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + i * 50);
		}
		g.drawText(s, this.x + this.width/2 - 30, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + 200);
		g.drawText(s, this.x + this.width - 30, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + 200);
		s = "-10";
		for (int i = 0; i < 2; i++) {
			g.drawText(s, this.x + 10, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + i * 50);
		}
		s = "+10";
		for (int i = 0; i < 2; i++) {
			g.drawText(s, this.x + width - 40, this.y + 25 - ApoHybridPanel.game_font.mCharCellHeight/2 + i * 50);
		}
		
		int middle = (int)(this.x + this.width/2);
		s = "radius";
		g.drawText(s, middle - ApoHybridPanel.game_font.getLength(s)/2, this.y + 0);
		
		s = "angle";
		g.drawText(s, middle - ApoHybridPanel.game_font.getLength(s)/2, this.y + 50);
		
		s = "rotation speed";
		g.drawText(s, middle - ApoHybridPanel.game_font.getLength(s)/2, this.y + 100);
		
		
		s = "rotate clockwise";
		g.drawText(s, middle - ApoHybridPanel.game_font.getLength(s)/2, this.y + 150);
		
		s = "x";
		g.drawText(s, middle - this.width/4 - ApoHybridPanel.game_font.getLength(s)/2, this.y + 195);
		
		s = "y";
		g.drawText(s, middle + this.width/4 - ApoHybridPanel.game_font.getLength(s)/2, this.y + 195);
		
		g.setFont(ApoHybridPanel.font);
